...
class Storage:
    ...
    storage_estimate: dict[str, float] = field(default_factory=dict)  # Storage usage and quota estimates // navigator.storage.estimate()
...
class PerformanceTimings:
    timings: dict[str, float] = field(default_factory=dict)         # Performance timing metrics // performance.getEntriesByType('navigation')
...
fingerprint = Fingerprint(
    ip=IP(
        ...
        details={"isp": "Example ISP", "country": "US", "city": "Example City"}
    ),
    audio=Audio(
        audio_hash="abc123def456audiohash"
    ),
    behavioral=Behavioral(
        typing_speed=45.7,
        mouse_entropy=0.78,
        keystroke_dynamics={"keyA": 0.12, "keyB": 0.15},
        scroll_behavior={"scrollSpeed": 1.2, "scrollFrequency": 7},
        url_changes=["https://example.com", "https://example.com/page2"],
        time_of_visit_patterns=["morning", "weekday"]
    ),
    browser=Browser(
        vendor="Google Inc.",
    ...
    canvas=Canvas(
        canvas_hash="canvashashmocked1234567890abcdef",
        webgl_hash="webglhashmockedabcdef1234567890"
    ),
    css_features=CSSMediaFeature(
    ...
    fonts=Fonts(
        installed_fonts=["Arial", "Helvetica", "Times New Roman"]
    ),
    graphics=Graphics(
        webgl_renderer="ANGLE (NVIDIA GeForce GTX 1060 Direct3D11 vs_5_0 ps_5_0)",
        webgl_vendor="NVIDIA Corporation",
        webgl_extensions=["EXT_texture_filter_anisotropic", "OES_element_index_uint"],
        webgpu_adapter={"name": "NVIDIA GeForce GTX 1060", "features": ["feature1", "feature2"], "limits": {}, "isFallbackAdapter": False}
    ),
    hardware=Hardware(
        os="Windows 10 Pro 10.0.19044",
        cpu_cores=8,
        device_memory=16.0,
        device_architecture="x86_64"
    ),
    http_header_fingerprint=HttpHeaderFingerprint(
        header_count=12,
        http_version="HTTP/2",
        ...
        headers_present=["host", "user-agent", "accept", "accept-encoding", "accept-language"],
        unusual_headers=["x-custom-header"]
    ),
    media=Media(
        audio_codecs=["opus", "mp3", "aac"],
        video_codecs=["vp8", "vp9", "h264"],
        media_devices=[
            {"deviceId": "default", "kind": "audioinput", "label": "Microphone"},
            {"deviceId": "default", "kind": "videoinput", "label": "Webcam"}
        ]
    ),
    network=NetworkConnection(
        effective_type="4g",
    ...
    performance=PerformanceTimings(
        timings={"loadEventEnd": 1234.5, "domContentLoadedEventEnd": 567.8},
        memory={"jsHeapSizeLimit": 2190000000, "totalJSHeapSize": 1800000000, "usedJSHeapSize": 1400000000},
        network_timing={"resourceFetchStart": 100.5, "resourceResponseEnd": 350.6}
    ),
    permissions=PermissionsStatus(
        geolocation="granted",
    ...
    plugins=Plugins(
        installed_plugins=["Flash", "Widevine Content Decryption Module"],
        mime_types=["application/pdf", "application/x-shockwave-flash"]
    ),
    encrypted_media_capabilities=EncryptedMediaCapabilities(
        cdm_list=["com.widevine.alpha", "com.microsoft.playready"]
    ),
    storage=Storage(
        cookies_enabled=True,
        storage_estimate={"usage": 1024.5, "quota": 5000.0},
        service_workers=["https://example.com/sw.js"],
        indexeddb_dbs=["db1", "db2"],
        cache_storage_keys=["cache1", "cache2"]
    ),
    time_zone=TimeZone(
...
for field in fingerprint.__dataclass_fields__:
